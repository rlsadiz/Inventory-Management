...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			var validateDiscount;
			for (var x = 0; x < discount.length; x++) {       
					var discountId = discount[x].id;
				if(discount[x].value == ''){	    	
					$("#"+discountId+"").after('<p class="text-danger"> Discount Field is required!! </p>');
					$("#"+discountId+"").closest('.form-group').addClass('has-error');	    		    		    	
			  } else {      	
					$("#"+discountId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
			  } 
			}  // for		
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentStatus) {
				if(validateProduct == true && validateDiscount == true && validateQuantity == true) {
					// create order button
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;">'+
					'<input type="number" step="0.01" name="discount[]" id="discount'+count+'" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control" />'+
				'</td>'+
