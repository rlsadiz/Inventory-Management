	...
	if(brandId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// brand id 
				$(".editBrandFooter").after('<input type="hidden" name="brandId" id="brandId" value="'+response.id_brand+'" />');
...
function removeBrands(brandId = null) {
	if(brandId) {
		...
		$.ajax({
			...
			dataType: 'json',
			success:function(response) {
				$('.removeBrandFooter').after('<input type="hidden" name="removeBrandId" id="removeBrandId" value="'+response.id_brand+'" /> ');
